...
static CGFloat const kJPSThumbnailAnnotationViewExpandOffset      = 100.0f;
...
-(void)setupPoint
{
    _point = [[UIImageView alloc] initWithFrame:CGRectMake(55, 0, 17, 17)];
    [self addSubview:_point];
}

- (void)setupImageView {
    _imageView = [[UIImageView alloc] initWithFrame:CGRectMake(11.5f, 10.f, 52.0f, 52.0f)];
...
- (void)setupTitleLabel {
    _titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(20.0f, 16.0f, 76.0f, 20.0f)];
...
- (void)setupSubtitleLabel {
    _subtitleLabel = [[UILabel alloc] initWithFrame:CGRectMake(20.0f, 36.0f, 76.0f, 20.0f)];
...
- (void)updateWithThumbnail:(JPSThumbnail *)thumbnail {
    ...
    self.point.image = thumbnail.point;
}
...
- (void)animateBubbleWithDirection:(JPSThumbnailAnnotationViewAnimationDirection)animationDirection {
    ...
    [UIView animateWithDuration:kJPSThumbnailAnnotationViewAnimationDuration animations:^{
        CGFloat xOffset = (growing ? -1 : 1) * kJPSThumbnailAnnotationViewExpandOffset/2.0f;
        self.point.frame = CGRectOffset(self.point.frame, xOffset, 0.0f);
    } completion:^(BOOL finished) {
        if (animationDirection == JPSThumbnailAnnotationViewAnimationDirectionShrink) {
            self.state = JPSThumbnailAnnotationViewStateCollapsed;
        }
